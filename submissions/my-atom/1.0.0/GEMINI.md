# TinyAtom atom workspace: My Atom

This is the editable workspace for atom `my-atom`. Stay inside this folder.
Read the shared Studio docs in the parent folder before changing files:
`../CREATOR_GUIDE.md`, `../TINYATOM_README.md`, `../TINYATOM_API.md`, `../TINYATOM_CREATION_FLOW.md`, `../TINYATOM_TICKETS.md`, `../TINYATOM_CAPABILITIES.md`, `../TINYATOM_THEME.md`, and `../TINYATOM_WORKSPACES.md`.
...
- When you generate `.tinyatom/creation/tickets.json`, it MUST match this exact schema or Studio shows "The tickets file could not be read" (full ticket workflow in `../TINYATOM_TICKETS.md`):
tickets.json schema — Studio validates it; an off-schema file shows "The tickets file could not be read".
File shape: { "version": 1, "updatedAt": "<ISO-8601 string>", "tickets": [ <ticket>, … ] }.
Every new ticket object REQUIRES these five fields:
  - "id": short unique string, e.g. "T01".
  - "status": one of "backlog" | "in_progress" | "in_review" | "changes_requested" | "done" (exact strings, underscores — NOT "todo"/"in-progress").
  - "priority": one of "low" | "medium" | "high".
  - "file": exactly "tickets/<name>.md" (also create that markdown file under .tinyatom/creation/tickets/).
  - "sourceFiles": every package-relative implementation file this ticket owns or changes; use [] when none exist yet.
The markdown file holds ONLY human-readable detail (goal, tasks, acceptance, notes) — it MUST NOT carry a
status/state line. Status lives ONLY in tickets.json and is HOST-owned: create every ticket as "backlog"
and never edit a status afterwards — the host moves tickets when you publish a completion claim envelope.
Optional fields (use these names, not your own): "title" (string), "summary" (string), and the string
arrays "requirements", "prdSections", "lldSections", "capabilities", "runtimeDependencies", and "blockedBy".
sourceFiles lists every package-relative implementation file the ticket owns or changes;
declare shared files on every ticket that changes them so later edits invalidate only affected approvals.
(dependencies go in "blockedBy" as ticket ids — do NOT invent a "dependsOn"/"prd" field).
Do NOT add "review", "comments", or "qa" — the reviewers own those; leave them out.
Do NOT invent fields or status values. Example ticket:
  {"id":"T01","title":"Scaffold the recorder","status":"backlog","priority":"high","file":"tickets/T01-scaffold.md","summary":"Set up the atom skeleton.","requirements":["R1"],"prdSections":[],"lldSections":[],"capabilities":["screen-capture"],"runtimeDependencies":[],"sourceFiles":["index.html","src/recorder.js"],"blockedBy":[]}
- Do not edit `.tinyatom/creation/status.json`; Studio owns approvals.

## Source Workspace And Runtime Data

- This folder is the editable source workspace at `studio/my-atom/`. It contains app source, creation docs, and assets intentionally packaged with the atom.
- Studio Preview and installed atoms share a durable artifact workspace at `artifacts/my-atom/`. In the default TinyAtom library this is `~/TinyAtom/artifacts/my-atom`; from the default source workspace it is reachable as `../../artifacts/my-atom`.
- `window.tinyAtom.files`, `storage`, `media`, and `speech` operate on the artifact workspace. `window.tinyAtom.files.reference` stores external file refs in `artifacts/my-atom/.tinyatom/refs.json`, but referenced files stay in the user-picked location. A runtime path such as `resources/datasets/request.json` means `artifacts/my-atom/resources/datasets/request.json`, not a file beside `app.js`.
...
4. Break the PRD and LLD into tickets in `.tinyatom/creation/tickets.json` (the schema Studio validates is in `TINYATOM_TICKETS.md`), then implement one ticket at a time and keep each ticket current.
